...
public interface EventRepository extends JpaRepository<Event, UUID> {
    ...
    //pageable for listing large items
    Page<Event> findByOrganizerId(UUID organizerId, Pageable pageable);
    Optional<Event> findByIdAndOrganizerId(UUID id, UUID organizerId);

    //Derived method For listing all published event by status using Pages
    Page<Event> findByStatus(EventStatusEnum status, Pageable pageable);

    //special search in postgres using string query in published events (event name,venue)
    ...
    //to find by id and status in published events
